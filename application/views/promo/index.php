...
<ul class="thumbnails well">
  <?php if ($games): ?>
    ...
    <?php foreach ($games as $item): ?>
      <?php if (in_array($item->platform_id, array(2,5,7,8, 14))): ?>
        <li class="span2">
          <div class="thumbnail text-center">
            ...
            <?php  
              ...
              if (in_array($item->platform_id, array(7, 8, 14))) {
                $url .= 'android';
              }
              ...
              if (in_array($item->platform_id, array(8, 5, 14))) {
                $url .= 'tablet';
              }
              $url .= "/os/2/version/2.1";
            ?>
            <a class="open-window" style="width:64px;display:inline-block" href="<?php echo $url ?>">
              <img style="max-height:128px" src="<?php echo $item->logo ? base_url() . 'uploads/original/'.$item->logo : 'http://placekitten.com/64/64' ?>" alt="">
            </a>
          </div>
        </li>
      <?php endif ?>
